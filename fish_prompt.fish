...
function fish_prompt
  ...
  set -l envcolor (set_color 8f68b6)
  ...
  if set -q VIRTUAL_ENV
    set -g virtual_env "("(basename "$VIRTUAL_ENV")") "
  else
    set -g virtual_env "" 
  end

  # Line 1
  echo -n $pcolor'┌'$envcolor$virtual_env$cyan$USER$normal'@'$cyan$__fish_prompt_hostname $gray(prompt_pwd)$normal
  __fish_git_prompt
  ...
  echo -en $gray" "(date +%H:%M:%S)$normal
  echo
